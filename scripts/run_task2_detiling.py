...
task2_yolo_labels_dir = os.path.join(cfg.YOLO_PATH, "predict_project_task_2", "labels")
detiled_task2_output_dir = os.path.join(
    cfg.YOLO_PATH, "results", "detiled_predictions_task_2"
)

os.makedirs(task2_yolo_labels_dir, exist_ok=True)
os.makedirs(detiled_task2_output_dir, exist_ok=True)

untile_predictions(
    test_txt_path,
    task2_yolo_labels_dir,
    original_images_dir,
    detiled_task2_output_dir,
    cfg.TILE_SIZE,
    cfg.OVERLAP,
)

# Draw boxes on all detiled pages
os.makedirs(os.path.join(detiled_task2_output_dir, "visualized_task_2"), exist_ok=True)

all_original_images = sorted(
    [f for f in os.listdir(original_images_dir) if f.endswith(".jpg")]
)

for result_txt in sorted(os.listdir(detiled_task2_output_dir)):
    ...
    annotation_path = os.path.join(detiled_task2_output_dir, result_txt)
    output_path = os.path.join(
        detiled_task2_output_dir,
        "visualized_task_2",
        result_txt.replace(".txt", "_task2.jpg"),
    )
